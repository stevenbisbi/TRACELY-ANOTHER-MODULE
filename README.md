...
| **Docente** | `/teacher/courses` ("Mis Cursos") | Toma asistencia del día (presente/ausente) y ve el % de asistencia acumulado de cada estudiante — **eso sí existe y funciona**. Lo que no tiene es visibilidad del *módulo de excusas* en sí: no ve si una falta puntual fue justificada por una excusa avalada, ni el detalle día por día, ni el análisis de la IA. Cuando una excusa se avala, la inasistencia queda marcada como justificada en la base de datos (`asistencia.justificada`), pero esa marca todavía no se refleja en su pantalla — solo ve el número resumido de asistencia. |

En resumen: la IA (extracción del certificado + evaluación contra el reglamento) solo es visible para la Dirección, que es quien decide; el estudiante ve el resultado, no el razonamiento; y el docente ve asistencia normalmente, pero sin el desglose de qué faltas quedaron justificadas por excusa.
